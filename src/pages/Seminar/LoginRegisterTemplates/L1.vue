          ...
          <div class="container my-auto py-5">
            <div class="row">
              <div class="col-11 col-lg-9 col-xl-8 mx-auto">
                <h3 class="font-weight-600 q-py-md">{{project.login_settings.translate.loginTitle[$route.params.lang]}}</h3>
                <h6 class="font-weight-300 text-red-8 no-margin">{{error}}</h6>
                <div v-if="project.login_type.value !== 'register_sms'">
                  <div class="form-group">
                    <input type="email" class="form-control" v-if="project.register_form.use_own_password || project.register_form.use_common_password" v-model="loginData.email" required :placeholder="project.login_settings.translate.enterEmail[$route.params.lang]" autocomplete="off">
                  </div>
                  <div class="form-group">
                    <input type="password" class="form-control" id="loginPassword" v-model="loginData.password" required="" :placeholder="project.login_settings.translate.enterPassword[$route.params.lang]" autocomplete="off">
                  </div>
                  <div class="row" v-if="project.register_form.use_own_password && project.show_forget_password_at_login">
                    <div class="col-sm text-right"><router-link :to="{ name: 'live.forgotPassword', params: { lang: this.$route.params.lang } }" class="btn-link" href="#"> {{project.login_settings.translate.forgotPassword[$route.params.lang]}} </router-link></div>
                  </div>
                </div>
                <div v-else>
                  <div class="form-group">
                    <input class="form-control" v-if="!sms_sent" v-model="loginData.phone" required :placeholder="$t('inputs.phone')" autocomplete="off">
                    <input class="form-control" v-else v-model="loginData.sms_code" required :placeholder="$t('inputs.smsCode')" autocomplete="off">
                  </div>
                </div>
                <p class="text-red-8" v-if="(errors && errors.message)">{{errors.message[0]}}</p>
                <button v-if="project.login_type.value !== 'register_sms'" :disabled="saving || fetching" class="btn btn-primary btn-block my-4"  :style="{'cursor':'pointer', 'background-color': project.btn_color ? project.btn_color : '#007bff'}" type="button" @click="submit" >{{ (saving || fetching) ?  project.login_settings.translate.pleaseWait[$route.params.lang] : project.login_settings.translate.login[$route.params.lang] }}</button>
                <button v-else-if="project.login_type.value === 'register_sms' && !sms_sent" :disabled="saving || fetching" class="btn btn-primary btn-block my-4"  :style="{'cursor':'pointer', 'background-color': project.btn_color ? project.btn_color : '#007bff'}" type="button" @click="sendCode" >{{ (saving || fetching) ? project.login_settings.translate.pleaseWait[$route.params.lang] : project.login_settings.translate.sendCode[$route.params.lang]   }}</button>
                <button v-else-if="project.login_type.value === 'register_sms' && sms_sent" :disabled="saving || fetching" class="btn btn-primary btn-block my-4"  :style="{'cursor':'pointer', 'background-color': project.btn_color ? project.btn_color : '#007bff'}" type="button" @click="codeLogin" >{{ (saving || fetching) ? project.login_settings.translate.pleaseWait[$route.params.lang] : project.login_settings.translate.login[$route.params.lang]   }}</button>
                <p class="text-center text-muted mb-0" v-if="project.show_register_link_at_login"><router-link class="btn-link" :to="{ name: 'live.register', params: { lang: this.$route.params.lang } }">{{  project.login_settings.translate.registerNow[$route.params.lang] }}</router-link></p>
              </div>
            </div>
            <div>
            </div>
...
export default {
  ...
  data () {
    return {
      ...
      error: '',
      sms_sent: this.$route.query.sms === 'o'
    }
  },
  computed: {
    errors () {
  ...
  methods: {
    ...
    sendCode () {
      const self = this
      this.$store
        .dispatch('projects/sendSMSCode', this.loginData).then(res => {
          this.sms_sent = true
        }).catch(() => {
          this.error = this.$t('login.invalidPhone')
          setTimeout(function () {
            self.error = ''
          }, 5000)
        })
    ...
    submit () {
      const self = this
      this.$store
        .dispatch('projects/login', this.loginData).then(res => {
          if (this.project.register_form.use_own_password || this.project.register_form.use_common_password) {
            this.$router.push({ name: 'live.registered_pass', params: { pass: res.__hash, lang: this.$route.params.lang } })
          } else {
            this.$router.push({ name: 'live.pass', params: { pass: this.loginData.password, lang: this.$route.params.lang } })
          }
        }).catch(() => {
          this.error = this.$t('login.invalidPassword')
          setTimeout(function () {
...
.btn-primary:focus{color:#fff;background-color:#0069d9;box-shadow:0 0 0 .2rem rgba(38,143,255,.5);}
.btn-primary:disabled{color:#fff;background-color:#007bff;}
